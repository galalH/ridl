...
#' @param date_range_start Data collection first date(*) - Use dd/mm/yyyy format.
#' @param date_range_end Data collection last date(*) - Use dd/mm/yyyy format.
...
resource_create <- function(pkgid, metadata) {
  ridl("resource_create", package_id = pkgid, clear_upload = "", url_type = "upload", !!!metadata)$results %>%
    as_tibble()
}
...
#' @rdname resource
#' @export
resource_update <- function(id, metadata) { ridl("resource_update", id = id, !!!metadata)$results %>% as_tibble() }

#' @rdname resource
#' @export
resource_patch <- function(id, metadata) { ridl("resource_patch", id = id, !!!metadata)$results %>% as_tibble() }

#' @rdname resource
#' @export
resource_delete <- function(id) { ridl("resource_delete", id = id); NULL }

#' @rdname search
#' @export
resource_search <- function(q = NULL, rows = NULL, start = NULL) {
  ridl("resource_search", !!!(as.list(match.call()[-1])))$results %>% as_tibble()
}
