...
#' @return The dataset.
#' @export
package_create <- function(metadata) { ridl("package_create", !!!metadata)$results %>% as_tibble() }

#' @rdname package
#' @export
package_show <- function(id) { ridl("package_show", id = id)$results %>% as_tibble() }

#' @rdname package
#' @export
package_update <- function(id, metadata) { ridl("package_update", id = id, !!!metadata)$results %>% as_tibble() }

#' @rdname package
#' @export
package_patch <- function(id, metadata) { ridl("package_patch", id = id, !!!metadata)$results %>% as_tibble() }
...
package_search <- function(q = NULL, rows = NULL, start = NULL) {
  ridl("package_search", !!!(as.list(match.call()[-1])))$results %>% as_tibble()
}
